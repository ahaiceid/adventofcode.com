...
class TestSolver(unittest.TestCase):
    ...
    def test_moves(self):
        for state_before, move_descriptor, state_after in self.move_cases:
            with self.subTest(msg=move_descriptor):
                head_before, tail_before = decode_state(state_before)
                head_after, tail_after = decode_state(state_after)
                move_direction, move_count = move_descriptor.split()
                rope_bridge = solver.RopeBridge(head_before, tail_before)
                for _ in range(int(move_count)):
                    rope_bridge.move_head(move_direction)
                    rope_bridge.update_tail()
                self.assertEqual(tuple(rope_bridge.head_position), head_after)
                self.assertEqual(tuple(rope_bridge.tail_position), tail_after)
                self.assertEqual(rope_bridge.head_position[0], head_after[0])
                self.assertEqual(rope_bridge.head_position[1], head_after[1])
                self.assertEqual(rope_bridge.tail_position[0], tail_after[0])
                self.assertEqual(rope_bridge.tail_position[1], tail_after[1])
    ...
    def test_visited(self):
        ...
        visited_positions.add(tuple(rope_bridge.tail_position))
